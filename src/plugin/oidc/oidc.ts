...
export const oidcFantocci: FastifyPluginAsync<OIDCFantocciOptions> =
  async function (
    fastify,
    { tokenHeader, discovery, clientId, clientSecret, issuer }
  ) {
    const oidcIssuer: Issuer = await Issuer.discover(
      resolveWellKnownUri(discovery ?? issuer)
    );
...
function getToken(
  ...
  if (tokenHeader) {
    const tokenHeaderValue =
      headers[tokenHeader.toLowerCase() as Lowercase<string>];
    if (tokenHeaderValue !== undefined && typeof tokenHeaderValue !== 'string')
      return;
